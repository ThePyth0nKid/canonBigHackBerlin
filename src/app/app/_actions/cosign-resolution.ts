...
> {
  ...
  const cosignerEmail = (admin.email || '').toLowerCase();
  ...
  const sourceRef = `resolution:cosign:${admin.userId}`;
  ...
  const r = await prisma.$transaction([
    prisma.factEvent.create({
      data: {
        ...
        notes: `cosign-of:${original.eventHash}:by:${cosignerEmail || admin.userId}`,
      },
    }),
    prisma.factEvent.update({
      ...
      data: {
        status: 'resolution',
        notes: `${original.notes ?? ''};cosigned-by:${signRes.eventHash}:by:${cosignerEmail || admin.userId}`,
      },
    }),
  ...
  return {
    ...
    cosignerEmail: cosignerEmail || admin.userId,
  };
}
